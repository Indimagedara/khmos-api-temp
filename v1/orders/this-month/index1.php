...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    WHERE O.EmployeeId = '$employeeId' AND U.Status = 1 AND    
    STR_TO_DATE(OrderDate, '%m/%d/%Y') BETWEEN '$fromDate' AND '$toDate'
    ORDER BY O.OrderId ASC";

    $amountQuery = "SELECT * FROM `amounts`";
    $amountResult = mysqli_query($con, $amountQuery);

    if ($result = mysqli_query($con, $query)) {
        $orderCountForThisMonth = mysqli_num_rows($result);
        $i = 0;
        while ($rows = mysqli_fetch_assoc($result)) {
            ...
            $amount = getAmouts($i, $orderCountForThisMonth, $amountResult, $orderType, $isVeg);
            // if($i < 50){
            // }
