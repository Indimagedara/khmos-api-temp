        ...
        foreach ($data as $row) {
            if (isset($row['orderType'], $row['userId'], $row['orderDate'])) {
                ...
                if(!isOrderExists($UserId, $OrderDate, $OrderType, $con)){
                    $query = "INSERT INTO `Orders` (`EmployeeId`, `OrderDate`, `Amount`, `IsVeg`, `OrderType`, `DivisionId`, `Location`,`OrderedBy`, `Status`, `DateCreated`, `DateModified`) 
                    VALUES ('$UserId', '$OrderDate', '$Amount', '$IsVeg', '$OrderType', '$DivisionId', '$SelectedLocation','$OrderedBy', '0', NOW(), NOW())";
